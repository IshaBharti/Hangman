...
def hint(secret_word,letters_guessed):
  ...
  while index<len(secret_word):
    if secret_word[index] not in letters_guessed:
      if secret_word[index] not in not_guessed:
        not_guessed.append(secret_word[index])
    index +=1
...
def hangman(secret_word):
    ...
    * Har round ke baar, user ko uska guess kiya hua partial word
    ...
    print ("")
    print("Your level")
    user=input("Enter a level  Easy, Medium, Hard:-")
    if user=="Easy":
      live=8
    elif user=="Medium":
      live=6
    ...
    while  lives > 0:  
      ...
      letter = guess.lower()
      # guess_word=hint(secret_word,letters_guessed)
      if guess=="hint":
